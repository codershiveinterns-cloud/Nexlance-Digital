const { buildCodaJsonRpcError, handleCodaTopup } = require('../services/coda');
...
module.exports = async function handler(req, res) {
    ...
    try {
        ...
        const payload = await handleCodaTopup({ rawBody, headers: req.headers });
        res.status(200).json(payload);
    } catch (error) {
        res.status(200).json(buildCodaJsonRpcError(null, -32000, error.message || 'Coda topup failed.'));
    }
};
